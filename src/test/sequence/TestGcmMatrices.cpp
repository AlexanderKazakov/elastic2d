...
TEST(Elastic3DModel, GcmMatrixInRotatedBasis) {
	...
	for (int i = 0; i < 10000; i++) {
		...
		const linal::Matrix33 b = linal::randomBasis(linal::Matrix33());
		const real l = Utils::randomReal(0.1, 10);
		
		Elastic3DModel::GcmMatrix m;
		Elastic3DModel::constructGcmMatrix(m, material, b, l);
		testTrace(m);
		testEigenstrings(m, 20 * EQUALITY_TOLERANCE, true);
		testEigenvectors(m, 20 * EQUALITY_TOLERANCE, true);
		testInverse(m);
		
		for (int j = 0; j < 3; j++) {
			Real3 n = Real3::Zeros();
			n(j) = 1; //< axis aligned basis
			Elastic3DModel::constructGcmMatrix(m, material,
					linal::createLocalBasis(n), l);
			testTrace(m);
			testEigenstrings(m, 10 * EQUALITY_TOLERANCE, true);
			testEigenvectors(m, 10 * EQUALITY_TOLERANCE, true);
			testInverse(m);
		}
	}
}
