...
//			const real eps = EQUALITY_TOLERANCE, const bool relative = false) {
//		for (int s = 0; s < DIMENSIONALITY; s++) {
//			testEigenvectors(matrix.m[s], eps, relative);
//		}
//	}
...
TEST(Elastic3DModel, GcmMatrixInRotatedBasis) {
	Utils::seedRand();
	for (int i = 0; i < 10; i++) {
		auto material = std::make_shared<const IsotropicMaterial>(
//				1, 2, 1);
				IsotropicMaterial::generateRandomMaterial());
		const linal::Matrix33 b = /*{
				1, 0, 0,
				0, 1, 0,
				0, 0, 1
		};*/
		linal::randomBasis(linal::Matrix33());
		const real l = 1; //Utils::randomReal(0.1, 10);
//		std::cout << material->rho << "\t" << material->lambda << "\t" << material->mu << "\t" << l << b;
		
		Elastic3DModel::GcmMatrix m;		
		Elastic3DModel::constructGcmMatrix(m, material, b, l);
		
		auto a = std::make_shared<Elastic3DModel::GCM_MATRICES>();
		Elastic3DModel::constructGcmMatrices(a, material);
//		std::cout << "A:" << a->m[2].A << m.A << "L:" << a->m[2].L << m.L << "U:" << a->m[2].U << m.U << "U1:" << a->m[2].U1 << m.U1;
