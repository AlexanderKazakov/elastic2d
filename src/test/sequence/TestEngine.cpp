...
TEST(Engine, TwoLayersDifferentRho) {
	...
	for (int i = 0; i < 5; i++) {
		...
		statement.materialConditions.defaultMaterial = std::make_shared<IsotropicMaterial>(
		        rho0, lambda0, mu0);
		Statement::MaterialCondition::Inhomogenity newMaterial;
		newMaterial.area = std::make_shared<AxisAlignedBoxArea>(Real3({-10, 0.5 - 1e-5,
		                                                               -10}),
		                                                        Real3({10, 10, 10}));
		...
		auto init = engine.getSolverForTest()->getMesh()->pde({task.cubicGrid.sizes(
		                                                               0) / 2,
		                                                       leftNodeIndex, 0});
...
//		int rightNodeIndex = (int) (task.cubicGrid.sizes(1) * 0.7);
		auto reflect = engine.getSolverForTest()->getMesh()->pde({task.cubicGrid.sizes(
		                                                                  0) / 2,
		                                                          leftNodeIndex, 0});
//		auto transfer = engine.getSolverForTest()->getMesh()->pde({task.cubicGrid.sizes(0) /
// 2, rightNodeIndex, 0});
...
TEST(Engine, TwoLayersDifferentE) {
	...
	for (int i = 0; i < 5; i++) {
		...
		real rho0 = 1, lambda0 = 2, mu0 = 0.8;
		real rho2rho0 = 1, lambda2lambda0 = E2E0Initial *
		                                    pow(2, i), mu2mu0 = E2E0Initial * pow(2, i);
		real rho = rho2rho0 * rho0, lambda = lambda2lambda0 * lambda0, mu = mu2mu0 * mu0;
