...
namespace gcm {
	...
	struct SnapshotterFactory<Binary2DSeismograph, TMesh, TModel, TGrid, TMaterial> {
		Snapshotter* create() {
			return create<TModel::DIMENSIONALITY == 2 && std::is_same<TGrid, CubicGrid>::value>();
		}
	...
	struct ModelFactory : public VirtualModelFactory {
		virtual VirtualMeshFactory* create(Models::T modelId) const override {
			switch (modelId) {
				...
				default:
					THROW_INVALID_ARG("The type of rheology model is unknown or unsuitable for specified type of grid and material");
			}
		}
	};
	...
	private:
		static VirtualProgramAbstactFactory* create(Materials::T materialId, Grids::T gridId, Models::T modelId) {
