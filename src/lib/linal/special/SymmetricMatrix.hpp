...
public:
	static const int SIZE = M * (M + 1) / 2;                 // size of storage in units of
			                                         // gcm::real
	real values[SIZE];
};
...
template<int TM>
void Matrix<TM, TM, SymmetricMatrixContainer<TM> >::
...
template<int TM>
Matrix<TM, TM, SymmetricMatrixContainer<TM> > Matrix<TM, TM, SymmetricMatrixContainer<TM> >::
transpose() const {
	return (*this);
}
...
template<int TM>
real Matrix<TM, TM, SymmetricMatrixContainer<TM> >::
trace() const {
	real ans = 0.0;
	for (int i = 0; i < TM; i++) {
		ans += (*this)(i, i);
